...
(dorun (for [x (range (- (last (sort first-10000-primes)) 100) (last (sort first-10000-primes)))
             :let [factors (prime-factors x)]]
         (do
           (fact "prime-factors returns a sorted list" (sort factors) => factors)
           (fact "checking that all items are prime with a list of known primes" (every? first-10000-primes factors) true)
...
(facts "running tests on 10 large numbers near 600851475140"
       (facts "multiplied factors  equal original number"
